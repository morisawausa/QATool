...
class Script(QATask):
	...
	def get_metrics(self, master, parameters):
		"""Given reference glyph, stores the '_top' anchor points of the accent and its case (.case, .sc) counterparts for reference."""
		reference = { }
	...
	def check_anchors(self, master, points, report):
		for category in points:
			# list of marks
			marks = self.marks[category]
	...
	def run(self, parameters, report):
		...
		for master in self.masters:
			...
			self.check_anchors(master, alignment_points, report)
